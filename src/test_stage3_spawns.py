...
def main():
    ...
    env = build_env(render=False, tick_skip=8, stage=3, difficulty_level=1)
    ...
    for i in range(10):
        print(f"\n--- Showing Stage 3 Random Spawn {i+1}/10 ---")
        
        # Reset the environment to generate a new Stage 3 seed
        env.reset()
        ...
        print("Freezing for 5 seconds to inspect spawn setup...")
        freeze_seconds = 5.0
        start_time = time.time()
        for _ in range(int(freeze_seconds * 120)):
            vis.render(0, 120, rsim.GameMode.SOCCAR, pad_states, b_state, car_data)
            target_time = start_time + (_ / 120.0)
            now = time.time()
            if target_time > now:
                time.sleep(target_time - now)
        
        # Simulate physics for 5 seconds so we can see the ballistic arc and drive
        # Calculate car speed
        try:
        ...
        print("Simulating physics for 5 seconds...")
        sim_seconds = 5.0
        start_time = time.time()
